...
def _allow_domain(monkeypatch, domain="en.wikipedia.org"):
...
class TestFetchSourceBasic:
    ...
    def test_injects_base_tag_in_head(self, monkeypatch):
        _allow_domain(monkeypatch)
        monkeypatch.setattr(proxy.requests, "get", lambda url, timeout, headers=None: FakeResponse(b"<html><head><title>Test</title></head><body><img src=\"/images/logo.png\"></body></html>", url="https://en.wikipedia.org/wiki/Test"))
        result = proxy.fetch_source(PUBLIC_URL)
        assert result["status"] is True
    ...
    def test_google_books_bypasses_fetch(self, monkeypatch):
        _allow_domain(monkeypatch, "books.google.com")
        mock_get = MagicMock()
    ...
    def test_subscription_sites_return_fallback(self, monkeypatch, domain):
        _allow_domain(monkeypatch, domain)
    ...
    @pytest.mark.parametrize("status,error_substr,has_fallback", [
        (404, "failed to load source", False),
        (403, "blocked", True),
    ])
    def test_http_error_response(self, monkeypatch, status, error_substr, has_fallback):
        _allow_domain(monkeypatch, "example.org")
        monkeypatch.setattr(proxy.requests, "get", lambda url, timeout, headers=None: FakeResponse(b"", status_code=status))
        result = proxy.fetch_source(PUBLIC_URL)
        assert result["status"] is False
    ...
    def test_timeout(self, monkeypatch):
        ...
        _allow_domain(monkeypatch, "example.org")
        monkeypatch.setattr(proxy.requests, "get", lambda url, timeout, headers=None: (_ for _ in ()).throw(r.Timeout()))
        result = proxy.fetch_source(PUBLIC_URL)
        assert result["status"] is False
    ...
    def test_request_exception(self, monkeypatch):
        import requests as r
        _allow_domain(monkeypatch, "example.org")
        monkeypatch.setattr(proxy.requests, "get", lambda url, timeout, headers=None: (_ for _ in ()).throw(r.ConnectionError("refused")))
        result = proxy.fetch_source(PUBLIC_URL)
        assert result["status"] is False
...
class TestSanitization:
    def test_strips_executable_tags(self, monkeypatch):
        _allow_domain(monkeypatch)
        monkeypatch.setattr(proxy.requests, "get", lambda url, timeout, headers=None: FakeResponse(b"""<html><head><title>Test</title></head><body>
            <script>alert(1)</script><form action="/submit"><button>Go</button></form>
            <iframe src="https://evil.test"></iframe><input type="text" name="query">
            <embed src="https://evil.test/flash"><p>Legitimate paragraph</p>
        </body></html>"""))
        result = proxy.fetch_source(PUBLIC_URL)
        soup = BeautifulSoup(result["html"], "html.parser")
    ...
    def test_preserves_css_styles(self, monkeypatch):
        _allow_domain(monkeypatch, "www.nationalgeographic.com")
        monkeypatch.setattr(proxy.requests, "get", lambda url, timeout, headers=None: FakeResponse(b"""<html><head>
            <link rel="stylesheet" href="/css/main.css">
            <style>body { color: #333; font-family: serif; }</style>
        </head><body><div style="background: #f0f0f0; border: 1px solid #ccc;">Colored content</div></body></html>"""))
        result = proxy.fetch_source(PUBLIC_URL)
        soup = BeautifulSoup(result["html"], "html.parser")
    ...
    def test_preserves_nav_and_header_structure(self, monkeypatch):
        _allow_domain(monkeypatch, "www.britannica.com")
        monkeypatch.setattr(proxy.requests, "get", lambda url, timeout, headers=None: FakeResponse(b"""<html><head><title>Encyclopedia</title></head><body>
        ...
        result = proxy.fetch_source(PUBLIC_URL)
        soup = BeautifulSoup(result["html"], "html.parser")
        for tag in ("header", "nav", "footer", "article"):
            assert soup.find(tag) is not None
